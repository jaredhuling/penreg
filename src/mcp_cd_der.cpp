...
RcppExport SEXP coord_mcp_der(SEXP x_, 
                              SEXP y_, 
                              SEXP lambda_,
                              SEXP gamma_,
                              ...
                              SEXP opts_)
{
    ...
    // count how many should be 
    // included in the sum of squares loss part
    int num_loss = 0;
    for (int i = 0; i < penalty_factor.size(); ++i)
    {
        if (penalty_factor[i] == 0)
            num_loss++;
    }
    ...
    CoordMCPder *solver;
    solver = new CoordMCPder(datX, datY, penalty_factor, num_loss, tol);
